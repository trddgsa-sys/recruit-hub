...
export default function RecruiterCandidatesPage() {
  ...
  useEffect(() => {
    fetch('/api/applications?limit=50').then((r) => r.json()).then((data) => { if (data.success) setApps(data.data); setLoading(false); });
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      {apps.length === 0 ? (
        <Card><div className="py-12 text-center"><Users className="mx-auto h-12 w-12 text-gray-300 mb-4" /><p className="text-gray-500">No candidates have applied using your referral codes yet.</p></div></Card>
      ) : (
        <Card>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {apps.map((app) => (
                  <tr key={app.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{app.candidate?.user?.name ?? 'N/A'}</td>
                    <td className="px-4 py-3 text-gray-500">{app.candidate?.user?.email ?? 'N/A'}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400"><div>{app.job.title}</div><div className="text-xs text-gray-400">{app.job.company?.name}</div></td>

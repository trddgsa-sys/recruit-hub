...
export default function RecruiterAnalyticsPage() {
  ...
  useEffect(() => {
    fetch('/api/analytics/recruiter').then((r) => r.json()).then((res) => { if (res.success) setData(res.data); setLoading(false); });
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Applications Over Time</h2></CardHeader>
        <CardBody><ApplicationsLineChart data={data.applicationsOverTime} /></CardBody>
      </Card>
      {data.codePerformance.length > 0 && (
        <Card>
          <CardHeader><h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Code Performance</h2></CardHeader>

...
export default function AdminJobsPage() {
  ...
  const toggleStatus = async (id: string, currentStatus: string) => {
    ...
    await fetch(`/api/jobs/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus }) });
    fetchJobs();
  };
                ...
                <tr key={job.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => toggleFeatured(job.id, job.status)} className="p-1.5 rounded-lg text-gray-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20" title="Toggle featured"><Star className="h-4 w-4" /></button>
                      <button onClick={() => deleteJob(job.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}

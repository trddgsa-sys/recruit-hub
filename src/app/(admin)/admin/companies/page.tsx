...
export default function AdminCompaniesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: form.name, website: form.website || null, description: form.description || null, industry: form.industry || null, logo: form.logo || null }) });
      const data = await res.json();
      if (data.success) { setShowModal(false); fetchCompanies(); } else setError(data.error ?? 'Failed');
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {companies.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                  ...
                  <td className="px-4 py-3">{c.website ? <a href={c.website} target="_blank" className="text-indigo-600 hover:underline text-xs truncate">{c.website}</a> : '—'}</td>
                  <td className="px-4 py-3 text-gray-500">{c._count.jobs}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(c)} className="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Company' : 'New Company'} size="md">
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea className="block w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500" rows={3} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} />
          </div>

...
export default function AdminCandidatesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/candidates?${params}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.success) { setCandidates(data.data); setTotalPages(data.meta?.totalPages ?? 1); }
        setLoading(false);
      });
  }, [page, search]);
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {candidates.length === 0 ? (
                  ...
                  <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                    ...
                    <td className="px-4 py-3"><div className="flex flex-wrap gap-1">{c.skills.slice(0, 3).map((s) => <Badge key={s}>{s}</Badge>)}{c.skills.length > 3 && <Badge>+{c.skills.length - 3}</Badge>}</div></td>
                    <td className="px-4 py-3 text-gray-500">{c._count.applications}</td>
                    <td className="px-4 py-3">{c.resumeUrl ? <a href={c.resumeUrl} target="_blank" className="text-indigo-600 hover:underline text-xs">View</a> : <span className="text-gray-400 text-xs">None</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
